...
import { docGroups } from '@/pages/docs/docs-config';
    ...
    <aside className={cn('flex flex-col', className)}>
      ...
      <nav className="flex-1 overflow-y-auto">
        {filteredGroups.length === 0 ? (
          <p className="text-muted-foreground px-3 text-sm">Sin resultados.</p>
        ) : (
          filteredGroups.map((group) => {
            const Icon = group.icon;
            return (
              <div key={group.label} className="mb-6">
                <p className="text-muted-foreground/70 mb-2 flex items-center gap-1.5 px-3 text-xs font-medium tracking-wider uppercase">

...
export function RegisterForm() {
  ...
  return (
    ...
    >
      ...
      <TermsCheckbox register={register('acceptTerms')} error={errors.acceptTerms?.message} />
    ...
    <div className="space-y-1.5 pt-1">
      ...
      <p className="flex items-center justify-between text-xs">
        <span className="text-muted-foreground">
          Fuerza:{' '}
          <span className={cn('font-medium', labelColor[strength.score])}>{strength.label}</span>
        </span>
...
function TermsCheckbox({ register, error }: TermsCheckboxProps) {
  return (
    <div className="space-y-1.5">
      <label className="border-border/60 bg-background/30 hover:bg-background/60 flex cursor-pointer items-start gap-3 rounded-xl border p-3 text-sm transition-colors">
        <span className="relative mt-0.5 inline-flex h-4 w-4 shrink-0">
          <input
            type="checkbox"
            className="peer border-border bg-background checked:border-primary checked:bg-primary h-4 w-4 cursor-pointer appearance-none rounded-[5px] border transition-colors"
            {...register}
          />

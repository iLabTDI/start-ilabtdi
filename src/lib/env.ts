...
const envSchema = z.object({
  ...
  VITE_SUPABASE_ANON_KEY: z.string().min(20, 'VITE_SUPABASE_ANON_KEY parece inválida (muy corta)'),
  VITE_APP_NAME: z.string().default('iLab TDI'),
  VITE_APP_URL: z.string().url().default('http://localhost:5173'),
  VITE_APP_VERSION: z.string().default('0.1.0'),
  VITE_AUTH_BACKEND: z.enum(['supabase', 'php', 'demo']).default('supabase'),
...
/**
 * Modo demo: se activa explícitamente con VITE_AUTH_BACKEND=demo, o
 * automáticamente cuando el backend es Supabase pero la URL es placeholder.
 * En modo demo no se hacen llamadas reales y se usan cuentas hardcodeadas
 * con sesión persistida en localStorage.
 */

...
async function askQuestions(d: Answers): Promise<Answers> {
  ...
  answers.appUrl = await input({
    message: 'URL pública del proyecto (puedes ponerla después)',
  ...
  answers.backend = await select<Answers['backend']>({
    ...
    choices: [
      ...
      {
        name: 'MySQL en GoDaddy · PHP backend propio',
        value: 'php',
      },
      {
  ...
  if (answers.backend === 'supabase') {
    ...
    if (hasKeys) {
      ...
      answers.supabaseAnonKey = await password({
        message: 'VITE_SUPABASE_ANON_KEY',
      });
  ...
  if (answers.backend === 'php') {
    const hasDb = await confirmP({
      message: '¿Ya creaste la DB y user en cPanel?',
    ...
    if (hasDb) {
      ...
      answers.dbPass = await password({ message: 'DB_PASS' });
    } else {
  ...
  const wantFtp = await confirmP({
    message: '¿Configurar FTP para deploy automático a GoDaddy?',
  ...
  if (wantFtp) {
    ...
    answers.ftpPass = await password({ message: 'FTP_PASS' });
...
function printSummary(a: Answers): void {
  ...
  console.log(`\n${b}─── Resumen ──────────────────────────────────────${r}`);
  row('Proyecto', a.appName);
  row('URL pública', a.appUrl);
  row('Backend', a.backend);

  if (a.backend === 'supabase') {
    row('Supabase URL', a.supabaseUrl || `${g}(se pondrá después)${r}`);
    row('Anon key', a.supabaseAnonKey ? `${g}(guardada)${r}` : `${g}(se pondrá después)${r}`);
  }
  if (a.backend === 'php') {
    row('DB host', a.dbHost || `${g}(pendiente)${r}`);
    row('DB name', a.dbName || `${g}(pendiente)${r}`);
    row('JWT secret', `${g}(autogenerado · ${a.jwtSecret.length} chars)${r}`);
  }

  if (a.ftpHost) {
    row('FTP host', a.ftpHost);
    row('FTP dir', a.ftpDir);
  } else {
    row('FTP deploy', `${g}(no configurado)${r}`);
  }
...
function row(key: string, value: string): void {
  console.log(`  ${C.gray}${key.padEnd(14)}${C.reset}  ${value}`);
}
...
function printDone(a: Answers): void {
  ...
  console.log(`\n${g}✓ Bootstrap completado.${r}\n`);
  console.log(`${C.bold}Siguientes pasos:${r}`);
  console.log(`  ${gr}1.${r} pnpm dev                ${gr}# dev server${r}`);

  if (a.backend === 'php' && (a.dbHost || a.dbName)) {
    console.log(`  ${gr}2.${r} pnpm db:setup           ${gr}# aplica migraciones MySQL${r}`);
  } else if (a.backend === 'supabase' && a.supabaseUrl) {
    console.log(`  ${gr}2.${r} Abre Supabase SQL Editor y corre supabase/_setup.sql`);
  }

  if (!a.ghPush && a.ftpHost) {
    console.log(
      `  ${gr}3.${r} Sube los secrets a GitHub manualmente ${gr}(ver /docs/subir-a-github)${r}`
    );
  }
...
function uploadGithubSecrets(a: Answers): void {
  ...
  log('info', `subiendo ${Object.keys(secrets).length} secrets${a.ghRepo ? ` a ${a.ghRepo}` : ''}…`);
